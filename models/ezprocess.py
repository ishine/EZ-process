...
class EZProcess(
        ...
        library_name="EZ-process",
        repo_url="https://github.com/WangHelin1997/EZ-process",
        tags=["text-to-speech"],
    ):
    ...
    def __init__(self, args: Optional[Namespace] = None, config: Optional[Dict] = None):
        ...
        self.n_prosody_tokens = self.args.prosody_vocab_size + 1
        assert self.args.prosody_pad_token == self.args.prosody_vocab_size, f"self.args.prosody_vocab_size: {self.args.prosody_vocab_size}, self.args.prosody_pad_token: {self.args.prosody_pad_token}"
        ...
        self.prosody_embedding = TokenEmbedding(
            dim_model=self.args.d_model,
            vocab_size=self.n_prosody_tokens, 
            dropout=self.args.prosody_embedding_dropout
        )
        ...
        self.timbre_embedding =  nn.Sequential(
                    nn.Linear(self.args.spk_emb, self.args.d_model // 2), 
                    nn.GELU(), 
        ...
        self.prosody_positional_embedding = SinePositionalEmbedding(
            self.args.d_model,
            dropout=self.args.prosody_positional_embedding_dropout,
            scale=False,
            alpha=True, # learnable scaler, scale the volume of positional embedding
        )
    ...
    def dec_forward(
            ...
            bsz, src_len = x_input.shape[0], x_lens.max() + p_lens.max() + t_lens.max() + new_y_lens.max()
            xpty_padding_mask = torch.concat([x_padding_mask, p_padding_mask, t_padding_mask, y_padding_mask], dim=1)
            _xpty_padding_mask = (
                xpty_padding_mask.view(bsz, 1, 1, src_len)
                .expand(-1, self.args.nhead, -1, -1)
                .reshape(bsz * self.args.nhead, 1, src_len)
            )
            # Check shapes and resize+broadcast as necessary
            if xpty_attn_mask.shape != _xpty_padding_mask.shape:
                assert xpty_attn_mask.ndim + 1 == _xpty_padding_mask.ndim, f"xpty_attn_mask.shape: {xpty_attn_mask.shape}, _xpty_padding_mask: {_xpty_padding_mask.shape}"
                xpty_attn_mask = xpty_attn_mask.unsqueeze(0).repeat(_xpty_padding_mask.shape[0], 1, 1)  # Example approach
            xpty_attn_mask = xpty_attn_mask.logical_or(_xpty_padding_mask)

            new_attn_mask = torch.zeros_like(xpty_attn_mask)
            new_attn_mask.masked_fill_(xpty_attn_mask, float("-inf"))
            xpty_attn_mask = new_attn_mask

            xpty_input = torch.cat([x_input, p_input, t_input, y_input], dim=1)

            if past == None: # do not use kvcache
                out, _ =  self.decoder((xpty_input, None), mask=xpty_attn_mask)
                return out[:, x_lens.max()+p_lens.max()+t_lens.max():], None
            else: # use kvcache
                if past.ndim > 3: # uses kvcache, only need to pass the last tokens, this doesn't work with multi-span speech editing yet
                    if last_3_tokens:
                        xpty_input = xpty_input[:, -3:]
                        xpty_attn_mask = xpty_attn_mask[:, -3:]
                    else:
                        xpty_input = xpty_input[:, -1:]
                        xpty_attn_mask = xpty_attn_mask[:, -1:]

                out, present = self.decoder((xpty_input, None), mask=xpty_attn_mask, past=past)
                if isinstance(out, tuple): # get rid of stage_embedding
                    out = out[0]

                if out.shape[1] > x_lens.max()+p_lens.max()+t_lens.max(): # the first pass, not kvcache yet
                    return out[:, x_lens.max()+p_lens.max()+t_lens.max():], present
                else: # used kvcache
                    return out, present
    ...
    def forward(self, batch):
        ...
        Args:
          ...
          p_lens:
            A 1-D tensor of shape (N,). It contains the number of tokens in `p`
            before padding.
          t:
            A 2-D tensor of shape (N, 1, V)
        """
        x, x_lens, y, y_lens, p, p_lens, t, t_lens = batch["x"], batch["x_lens"], batch["y"], batch["y_lens"], batch["p"], batch["p_lens"], batch["t"], batch["t_lens"]
        if len(x) == 0:
            return None
        ...
        x_attention_mask = torch.full((x.shape[1], x.shape[1]), 0).bool().to(x_padding_mask.device)
        ...
        t_input = self.timbre_embedding(t)
    ...
    def inference(
        ...
        Args:
          ...
          p:
            A 2-D tensor of shape (1, L).
          p_lens:
            A 1-D tensor of shape (1,). It contains the number of tokens in `p`
          t:
            A 2-D tensor of shape (1, L).
          t_lens:
            A 1-D tensor of shape (1,). It contains the number of tokens in `t`
          mask_interval:
        ...
        # make p attention mask and p_input
        p_attention_mask = torch.triu(torch.ones(p.shape[1], p.shape[1]), diagonal=1).bool().to(p.device)
        p_input = self.prosody_embedding(p)
        p_input = self.prosody_positional_embedding(p_input)
        
        # make t attention mask and t_input
        t_attention_mask = torch.triu(torch.ones(1, 1), diagonal=1).bool().to(t.device)
        t_input = self.timbre_embedding(t)
        ...
        # print(mask_intervals) # [(250, 425)]
        # print(non_mask_intervals # [(0, 250), (425, 656)]
        ...
        assert shifted_y[0].shape[0] == self.args.n_codebooks, shifted_y[0].shape
        # print([item.shape for item in shifted_y]) # [torch.Size([4, 254]), torch.Size([4, 235]), torch.Size([4, 179])]
        inserted_y, mask_position = self.insert_mask(shifted_y)
        # print([item.shape for item in inserted_y]) # [torch.Size([4, 254]), torch.Size([4, 1]), torch.Size([4, 235]), torch.Size([4, 1]), torch.Size([4, 179])]
        assert inserted_y[0].shape[0] == self.args.n_codebooks, inserted_y[0].shape[0]
        cated_y, new_y_lens = self.cat_y(inserted_y) # KT
        # print(y.shape, cated_y.shape) # torch.Size([1, 4, 656]) torch.Size([4, 670])
        assert not (cated_y == self.args.audio_pad_token).any(), cated_y
        num_task = len(mask_position)//2
        assert num_task == len(mask_position)/2, mask_position
        # print(mask_position[num_task]) # 490 = 254 + 1 + 235
        ...
        p_padding_mask = torch.full((1,p_lens[0]), False).to(p.device)
        t_padding_mask = torch.full((1,t_lens[0]), False).to(t.device)
        emb_inds = list(range(self.args.mts, self.args.mts+ self.args.max_n_spans))

        past = torch.ones([self.args.num_decoder_layers, 2, x.shape[0]], device=x.device, dtype=torch.float32) if kvcache else None # n_layers, 2, bsz, num_heads, src_len, head_dim
        generated = []
