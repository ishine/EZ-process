...
if __name__ == "__main__":
    ...
    codes_aug_save_root = os.path.join(args.save_dir, args.dataset_name, "encodec_16khz_4codebooks_prosody_speakingrate_augmented")
    prosody_timbre_save_root = os.path.join(args.save_dir, args.dataset_name, "codes_facodec")
    manifest_root = os.path.join(args.save_dir, args.dataset_name, "manifest_aug")
    ...
    for split in splits:
        ...
        for key in tqdm(range(len(jsondata))):
            if os.path.exists(os.path.join(phn_save_root, jsondata['segment_id'][key]+".txt")) and os.path.exists(os.path.join(codes_save_root, jsondata['segment_id'][key]+".txt")) and os.path.exists(os.path.join(codes_aug_save_root, jsondata['segment_id'][key]+".txt")) and os.path.exists(os.path.join(prosody_timbre_save_root, jsondata['segment_id'][key]+".pt")):
