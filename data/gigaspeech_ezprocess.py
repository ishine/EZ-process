...
class dataset(torch.utils.data.Dataset):
    ...
    def _load_phn_enc(self, index):
        ...
        af = os.path.join(self.args.dataset_dir, self.args.encodec_aug_folder_name, item[1]+".txt")
        tf = os.path.join(self.args.dataset_dir, self.args.prosody_timbre_folder_name, item[1]+".pt")
        # print(pf, ef, af, tf)
        try:
            with open(pf, "r") as p, open(ef, "r") as e, open (af, "r") as a:
                ...
                encos_aug = [l.strip().split() for k, l in enumerate(a.readlines()) if k < self.args.n_codebooks]
                assert len(encos) == self.args.n_codebooks, ef
                assert len(encos_aug) == self.args.n_codebooks, ef
                
                y = [[int(n) for n in l] for l in encos]
                z = [[int(n) for n in l] for l in encos_aug]
                p = torch.load(tf)['prosody']
                p_len = len(p)
                if p_len > int(self.args.prosody_max_length*self.args.encodec_sr):
                    start = random.randint(0, p_len-int(self.args.prosody_max_length*self.args.encodec_sr))
                    p = p[start:start+int(self.args.prosody_max_length*self.args.encodec_sr)]
                t = torch.load(tf)['timbre'].unsqueeze(0)
    ...
    def rearrange(self, y, z, non_mask_intervals, mask_intervals):
    
        assert self.args.eos > 0, f"eos={self.args.eos} should > 0"
        y_len = len(y[0])
        z_len = len(z[0])
        speaking_rate = float(z_len / y_len)
        ...
        for i, item in enumerate(non_mask_intervals):
            if i == 0:
                if item[0] == item[1]: # case: (0,0)
                    ...
                    start = max(0, int(item[0]*speaking_rate))
                    end = min(z_len, int(item[1]*speaking_rate))
                    rearranged_y.append(torch.cat([sos_tensor, z[:, start:end], y[:, item[1]:(item[1]+self.args.train_gap)]], dim=-1))
                ...
                if item[0] == item[1]: # case: (N,N)
                    ...
                    rearranged_y.append(torch.cat([y[:, (item[0]-self.args.train_gap):item[0]], z[:, start:end], eos_tensor], dim=-1))
            else:
                start = max(0, int(item[0]*speaking_rate))
                end = min(z_len, int(item[1]*speaking_rate))
                rearranged_y.append(torch.cat([y[:, (item[0]-self.args.train_gap):item[0]], z[:, start:end], y[:, item[1]:(item[1]+self.args.train_gap)]], dim=-1))
                
        for i, item in enumerate(mask_intervals):
            rearranged_y.append(torch.cat([y[:, (item[0]+self.args.train_gap): (item[1]-self.args.train_gap)], eog_tensor], dim=-1))
    ...
    def __getitem__(self, index):
        x, y, z, p, t = self._load_phn_enc(index)
        x_len, y_len, z_len, p_len = len(x), len(y[0]), len(z[0]), len(p)
    ...
    def collate(self, batch):
        ...
        if self.args.dynamic_batching:
            ...
            assert out['p'][0].ndim==1, out['p'][0].shape
            res['p'] = torch.nn.utils.rnn.pad_sequence(out['p'], batch_first=True, padding_value=self.args.prosody_pad_token)
        else:
            res['y'] = torch.stack(out['y'], dim=0)
            res['p'] = torch.stack(out['p'], dim=0)
        res["y_lens"] = torch.LongTensor(out["y_len"])
        res["p_lens"] = torch.LongTensor(out["p_len"])
        res["t"] = torch.stack(out["t"], dim=0)
        res["t_lens"] = torch.LongTensor(out["t_len"])
        return res
...
if __name__ == "__main__":
    ...
    args.exp_dir = "/apdcephfs_cq10/share_1603164/user/helinhwang/VoiceCraft/exp_results/English_copy/e830M_en"
    args.dataset = "English"
    args.dataset_dir = "/apdcephfs_cq10/share_1603164/data/Audio_Edit/VoiceCraft_English/English"
    val_dataset = dataset(args, 'validation')
    result = val_dataset[0]
    for i in range(100):
        print(max(val_dataset[i]["p"]))
    print(result["x"])
    print(result["y"][:, :20])
    print(result["p"])
    print(result["x"].shape, result["y"].shape, result["p"].shape, result["t"].shape, result["x_len"], result["y_len"], result["p_len"], result["t_len"])
